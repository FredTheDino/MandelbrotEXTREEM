...
{
	// Roll
	real roll = atan2(2.0f * (q.w * q.x + q.y * q.z), 1.0f - 2.0f * (q.x * q.x + q.y * q.y));
	//real roll = atan2(q.y * q.z + q.w * q.x, 0.5f - (q.x * q.x + q.y * q.y));
	...
	//real sinp = - 2 * (q.x * q.z - q.w * q.y);
	//real pitch = asin(sinp);

	// Yaw
	real yaw = atan2(2.0f * (q.w * q.z + q.x * q.y), 1.0f - 2.0f * (q.y * q.y + q.z * q.z));
	//real yaw = atan2(q.x * q.y + q.w * q.z, 0.5f - (q.y * q.y + q.z * q.z));
